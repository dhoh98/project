...
- 사용자의 진단된 투자 성향(`investment_type`)에 따라 **최적화된 펀드형 포트폴리오(10개 종목)**를 자동 추천
- 추천 로직은 `target_class` (내부 분류 기준), `연간변동성` 분위수, `CAGR` (연간 복리 성장률)을 기반으로 함
    - **안정추구형**: `target_class` 0 & 연간변동성 하위 25% 이내 종목 중 CAGR 상위 10개
    - **위험중립형**: `target_class` 1 & 연간변동성 하위 50% 이내 종목 중 CAGR 상위 10개
    - **적극투자형**: `target_class` 2 & 연간변동성 하위 75% 이내 종목 중 CAGR 상위 10개
    - **공격투자형**: `target_class` 3 & 연간변동성 전체(하위 100%) 종목 중 CAGR 상위 10개
- 추천된 펀드의 **평균 초과수익률, 평균 CAGR, 평균 연간변동성** 등 핵심 성과 지표를 요약하여 표시
- **"개별 종목 분석 및 포트폴리오 구성하기" 버튼**을 통해 추천된 종목들이 기본으로 선택된 상태로 다음 대시보드 페이지로 이동

### 📈 개별 종목 분석 및 포트폴리오 구성 (Individual Stock Analysis & Portfolio Building) - `pages/05_individual_stock_analysis.py`
- 전체 종목 리스트를 제공하며, 사용자가 직접 **투자성향 분류(`target_class`), 정렬 기준**(회사명, 배당수익률, 초과수익률, CAGR, 연간변동성 등), **정렬 순서**를 선택하여 종목을 탐색
- **종목명 검색 기능**으로 원하는 종목을 빠르게 검색
- `st.data_editor`를 활용하여 **체크박스로 관심 종목**을 포트폴리오에 쉽게 추가/제거 (`상위 5개 추가 선택`, `모두 해제` 버튼 제공)
- 선택된 포트폴리오의 **평균 초과수익률**을 계산하고, 종목별 수익률을 **막대그래프**로 시각화하여 분석 결과 제공
...
-   **데이터 처리 및 분석:**
    -   Pandas를 활용한 대규모 데이터 로딩, 전처리, 필터링, 정렬 기법 숙달.
    -   사용자 설문 기반의 투자성향 진단 로직(점수 계산, 유형 분류) 설계 및 구현.
    -   맞춤형 **펀드 추천 로직** 설계 (복합적인 필터링 및 정렬 기준 적용).
    -   재무 데이터를 기반으로 한 종목 필터링 및 포트폴리오 수익률 계산 로직 개발.

-   **데이터 시각화:**
    -   Plotly 라이브러리를 이용하여 동적이고 상호작용 가능한 막대그래프 및 게이지 차트 등 다양한 데이터 시각화 구현.
...
-   `회사명`
-   `거래소코드`
-   `회계년도` (2022년 데이터가 추천 로직에 활용됩니다.)
-   `이자보상배율(이자비용)`
-   `영업활동으로 인한 현금흐름(*)(천원)`
-   `초과수익률` (혹은 `초과수익률_apply`)
-   `연간변동성`
-   `CAGR` (연간 복리 성장률)
...
5.  **투자성향 맞춤 추천 펀드** (`pages/04_dashboard.py`): 진단 결과에 기반한 펀드형 추천 종목 리스트와 전체적인 펀드 성과를 확인합니다. 여기에서 개별 종목 분석 페이지로 이동할 수 있습니다.
